import { Menu, LogOut, User as UserIcon, KeyRound, Bell, Settings } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
            ...
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                <Avatar className="size-10 ring-1 ring-black/5">
                  ...
                  />
                  <AvatarFallback>
                    {(state.user?.name || state.user?.username || "U")
                      .slice(0, 1)
                      .toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                  <span className="text-[15px] font-semibold text-gray-800 leading-5">
                    {state.user?.name ||
                      state.user?.username ||
                      state.user?.email}
                  </span>
              ...
              <button
                ...
                className="relative grid place-items-center size-10 rounded-full border border-gray-300/70 bg-white"
              >
